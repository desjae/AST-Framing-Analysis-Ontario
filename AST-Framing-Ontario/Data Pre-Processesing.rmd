...
Instructions are provided below for processing a single folder of PDF documents, as a walkthough of the processing techniques used. Another set of instructions is provided at the end of this notebook that allows for the processing multiple folders simultaneously. 
...
Transporation consortia are dedicated transportation bodies that primarly focus on providing the school busing service to families in their associated region. 

In total, 9 webpages were pulled from 7 of the 39 consortia examined in Ontario. Webpages are determined to be relevent and specifically pretain/mention AST or STP. 


__Municipalities or Regions__ 

Municipalities or regions have webpages about transportation plans, programs, and services provided, some of which are not relevent to our analysis. Only webpages specifically dicussing active travel, AST, or STP were included. Webpages were also sourced from associated public health units becasues their direct relation to governing bodies at the municipal or regional level.

In total, 29 webpages were pulled. 

__School Boards__

These bodies use funding provided to them by the province to run a collection of schools. They prepare the annual budget, hire teachers and staff, supervise the education program, and other tasks ensure schools are following all regulations set by the *Education Act* [Ministry of Education]. Only the Englished based public and Catholic school boards were considered.

Pages mentioning STP and/or AST were prioritized. However, if they did not exist, pages discussing transportation were downloaded instead. In total, 32 webpages were pulled from 31 of the 61 school boards examined in Ontario. Of the remaining the were not included, 22 linked their respective corsortia, 7 did not have pages that meet the criteria, 1 met criteria but could not be downloaded in a processable format. 
...
PDF documents are brought into the corpus directly, however each page of the document is read as an invividual document. For our purposes, a conversion from PDF to .txt will allow us to keep all pages in a document together when loading it into the corpus. Make sure you have the directory path to where your documents are stored and Re-assign the 'pdfLocation' to that corresponding location. R uses '/' as the directory seporator. 

Location:

"E:/Github/AST-Framing-Analysis-Ontario/Texts/Academic Papers"
"E:/Github/AST-Framing-Analysis-Ontario/Texts/Consortia"
"E:/Github/AST-Framing-Analysis-Ontario/Texts/Municipalities or Regions"
"E:/Github/AST-Framing-Analysis-Ontario/Texts/School Boards"

```{r}
pdfLocation <- "E:/Github/AST-Framing-Analysis-Ontario/Texts/School Boards"
...
NAwords <- c("contact", "cancellations", "catholic", "waterloo", "board", "east", "transportation", "students", "student", "website", "tri", "can", "district", "will", "please", "may", "school", "https")
...
The goal of topic modelling is to identify the re-occurring themes in our corpus quantitatively with code. The code below utilizes LDA, an algorithm that when provided a number of desrete topics, 'k', will calculate the likelyhood of any given term being generated in a given topic. This algorithm runs under the 'bag of words' assumptions with means grammatic structre (adjective, noun, verb) is not considered. Words are rather considered 
```{r}
...
This is a more robust approach since LDA has some random compenent on how it begins developing its clusters. By taking the average of multiple iterations with random seeds we  
...
# Need to create new data frame with the cleaned corpus 
...
# Cuts out terms that have a minimum count of 5 
...
safe = word_vectors["mental", , drop = FALSE] 
...
Corpora will be cleaned for white spaces, punctuations, English stopwords, and converted to lowercases. Any additional stopwords will need to be removed manually as the user sees fit. 
```{r}
