...
Natural language processing [see @silgetextmining2021]
...
municipal_words <- municipal_vocab %>% slice(1939:1915)
school_words <- school_vocab %>% slice(1861:1837)
consortia_words <- consortium_vocab[-c(971),] %>% slice(993:968)
academic_words <- academic_vocab[-c(1309, 1304),] %>% slice(1311:1285)
```
...
# Import VCorpus created by tm package

policy_quanteda <- tm::VCorpus(tm::VectorSource(policy_corpus))
policy_quanteda <- corpus(policy_quanteda)

# Extraction of various search terms and the surrounding sentence context

kwic_pol_physical <- kwic(tokens(policy_quanteda), pattern = "physical")
kwic_pol_mental <- kwic(tokens(policy_quanteda), pattern = "mental")
kwic_pol_air <- kwic(tokens(policy_quanteda), pattern = "air")
kwic_pol_traffic <- kwic(tokens(policy_quanteda), pattern = "traffic")
kwic_pol_safety <- kwic(tokens(policy_quanteda), pattern = "safety")
kwic_pol_health <- kwic(tokens(policy_quanteda), pattern = "health")
kwic_pol_emissions <- kwic(tokens(policy_quanteda), pattern = "emissions")
kwic_pol_routes <- kwic(tokens(policy_quanteda), pattern = "routes")
kwic_pol_lanes <- kwic(tokens(policy_quanteda), pattern = "lanes")
kwic_pol_zone <- kwic(tokens(policy_quanteda), pattern = "zone")
kwic_pol_bus <- kwic(tokens(policy_quanteda), pattern = "bus")
kwic_pol_comm <- kwic(tokens(policy_quanteda), pattern = "community")
kwic_pol_ben <- kwic(tokens(policy_quanteda), pattern = "benefit")
kwic_pol_wea <- kwic(tokens(policy_quanteda), pattern = "weather")
kwic_pol_lanes <- kwic(tokens(policy_quanteda), pattern = "lanes")
```
...
municipal_lda_num <- FindTopicsNumber(
  municipal_dtm, # Check with other DTM
  topics = seq(from = 5, to = 20, by = 2),
...
school_lda_num <- FindTopicsNumber(
  school_dtm, # Check with other DTM
  topics = seq(from = 5, to = 20, by = 2),
  metrics = c("Griffiths2004", "CaoJuan2009", "Arun2010", "Deveaud2014"),
  method = "Gibbs",
  #control = list(seed = 77),
...
consortia_lda_num <- FindTopicsNumber(
  consortia_dtm, # Check with other DTM
  topics = seq(from = 5, to = 20, by = 2),
  metrics = c("Griffiths2004", "CaoJuan2009", "Arun2010", "Deveaud2014"),
  method = "Gibbs",
  #control = list(seed = 77),
...
policy_lda_num <- FindTopicsNumber(
  policy_dtm, # Check with other DTM
  topics = seq(from = 5, to = 20, by = 2),
  metrics = c("Griffiths2004", "CaoJuan2009", "Arun2010", "Deveaud2014"),
  method = "Gibbs",
  #control = list(seed = 77),
...
#academic_lda_num <- FindTopicsNumber(
  #academic_dtm, # Check with other DTM
...
# The output suggests that there are 9 topics in the municipal corpus
# The output suggests that there are 9 topics in the school corpus
# The output suggests that there are 7 topics in the municipal corpus
# The output suggests that there are 9 topics in the municipal corpus
# The output suggests that there are ? topics in the municipal corpus
```

```{r corpus-lda-model, echo=FALSE}
municipal_lda <- LDA(municipal_dtm, k = 9)
school_lda <- LDA(school_dtm, k = 9)
consortia_lda <- LDA(consortia_dtm, k = 7)
policy_lda <- LDA(policy_dtm, k = 9)
#academic_lda <- LDA(academic_dtm, k = ?)
```
...
The presence of _travel planning_ among the top pairs of consecutive words for municipalities, school boards, and transportation consortia reflects the priority given to this intervention in Ontario.

## 5.2. Framing of AST in academic papers

Notably absent from the policy corpora is _children's independent mobility_ (CIM). AST is one of the most common opportunities for children to travel without adult supervision on a regular basis, yet STP stakeholders do not discuss this topic. CIM was first described by Hillman et al. [-@hillmanOneFalseMove1990] as the freedom of children to travel and play within their neighborhood and city without the presence of adults. The ability of children to reach destinations such as school or local parks by walking or bicycling on their own is a growing area of research that is related to and often overlaps with the body of literature on AST. 

In a study conducted in multiple Canadian cities, Riazi et al. [-@riaziCorrelatesChildrenIndependent2019] found that a range of factors such as child grade, car ownership, and parental perceptions of safety and environment were associated with CIM. Parental perceptions of children's autonomy also influences CIM [@alparoneChildrenIndependentMobility2012]. In addition to independent trips to school, children also travel by walking or bicycling on their own to other non-school destinations such as outdoor spaces (i.e., parks, playgrounds), the homes of family members or friends, shopping places, and libraries, to name a few [@badlandAssessingNeighbourhoodDestination2015; @villanuevaWhereChildrenTravel2013a].
